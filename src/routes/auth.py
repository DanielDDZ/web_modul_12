...
async def login(body: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    contact = await repository_users.get_user_by_email(body.username, db)
    if contact is None:
        raise HTTPException(
    ...
    # Generate JWT
    access_token = await auth_service.create_access_token(data={"sub": contact.email}, expires_delta=7200)
...
async def refresh_token(credentials: HTTPAuthorizationCredentials = Security(security), db: Session = Depends(get_db)):
    ...
    contact = await repository_users.get_user_by_email(email, db)
